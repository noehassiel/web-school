    ...
    <div class="col-md-8">
        @switch($product->type)
            @case('physical')
                <!-- Inventory -->
                @if($product->has_variants == true)
                    @include('back.products.partials._variant_card')
                @endif

                @include('back.products.partials._relationship_card')
            @break
            ...
            @case('subscription')
                @include('back.products.partials._characteristics_card')
                @include('back.products.partials._relationship_card')
            @break
        @endswitch
        ...
        <div class="card mg-t-10 mb-4">
            <div class="card-header pd-t-20 pd-b-0 bd-b-0">
                ...
                @php
                    $logs = App\Notification::where('type', 'Producto')->where('model_id', $product->id)->get();
                @endphp
            </div>

            @if($logs->count() != 0)
                @include('back.layouts.partials._notification_table')
            @else
            <div class="card-body">
